...
Future<void> myBackgroundMessageHandler(RemoteMessage message) async {
  ...
  if (notificationBody != null && (notificationBody.notificationType == NotificationType.order || notificationBody.notificationType == NotificationType.order_request)) {
    ...
    // 1. Check if the driver has granted "Display over other apps"
    try {
      ...
      if (isPermissionGranted) {
        if (!await FlutterOverlayWindow.isActive()) {
          await FlutterOverlayWindow.showOverlay(
            ...
            overlayContent: 'Order #${message.data['order_id']}',
        ...
        await FlutterOverlayWindow.shareData(jsonEncode(message.data));
      }
    } catch (e) {
      customPrint("Overlay error: $e");
    }

    // 2. Continue with existing foreground task service
